...
export async function getFeaturedConcepts(): Promise<ConceptData[]> {
  return sql<ConceptData[]>`
    SELECT
      id, original_term, original_script, transliteration
    FROM concept
    WHERE status = 'published'
      AND original_term IS NOT NULL
    ORDER BY id
    LIMIT 8
  `;
}

export async function getHeroQuestion(
  locale: string,
): Promise<{ title_en: string; title_locale: string } | null> {
  const rows = await sql<
    { title_en: string; title_locale: string }[]
  >`
    SELECT
      COALESCE(t_en.value, '') AS title_en,
      COALESCE(t_loc.value, t_en.value, '') AS title_locale
    FROM question q
    LEFT JOIN translation t_en
      ON t_en.entity_type = 'question'
      AND t_en.entity_id = q.id
      AND t_en.locale = 'en'
      AND t_en.field = 'title'
    LEFT JOIN translation t_loc
      ON t_loc.entity_type = 'question'
      AND t_loc.entity_id = q.id
      AND t_loc.locale = ${locale}
      AND t_loc.field = 'title'
    WHERE q.id = 'q_DEATH'
    LIMIT 1
  `;
  return rows[0] ?? null;
...
export async function getTheRule(
  ...
  const rows = await sql<{ field: string; value: string }[]>`
    SELECT field, COALESCE(t.value, t_en.value) AS value
