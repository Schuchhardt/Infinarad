...
describe("schema integrity", () => {
  describe("citation quotability constraint (§5.5)", () => {
    it("allows quote on a quotable source (public_domain)", async () => {
      const result = await sql`
        SELECT quotable FROM infi_source WHERE id = 'src_DHAMMAPADA_PD'
      `;
      expect(result[0]?.quotable).toBe(true);

      const citations = await sql`
        SELECT id, quote FROM infi_citation
    ...
    it("rejects quote on a non-quotable source (proprietary)", async () => {
      const result = await sql`
        SELECT quotable FROM infi_source WHERE id = 'src_SMITH_WORLD'
      `;
      expect(result[0]?.quotable).toBe(false);

      await expect(
        sql`
          INSERT INTO infi_citation (id, source_id, source_quotable, locator, claim_text, quote)
          VALUES (
            'cit_TEST_FAIL',
            'src_SMITH_WORLD',
            false,
    ...
    it("allows claim without quote on a non-quotable source", async () => {
      const existing = await sql`
        SELECT id FROM infi_citation WHERE source_id = 'src_SMITH_WORLD' AND quote IS NULL
      `;
  ...
  describe("source quotable trigger (§5.4)", () => {
    it("auto-sets quotable based on license", async () => {
      const pd = await sql`SELECT quotable FROM infi_source WHERE license = 'public_domain' LIMIT 1`;
      expect(pd[0]?.quotable).toBe(true);

      const prop = await sql`SELECT quotable FROM infi_source WHERE license = 'proprietary' LIMIT 1`;
  ...
  describe("revision immutability (§5.6)", () => {
    it("has REVOKE on revision for UPDATE and DELETE", async () => {
      const result = await sql`
        SELECT has_table_privilege('public', 'infi_revision', 'UPDATE') AS can_update,
               has_table_privilege('public', 'infi_revision', 'DELETE') AS can_delete
      `;
  ...
  describe("translation system (§5.7)", () => {
    ...
    it("q_CONSCIOUSNESS has no ES translation (tests fallback)", async () => {
      const result = await sql`
        SELECT count(*)::int AS cnt FROM infi_translation
    ...
    it("translated view resolves fallback for q_CONSCIOUSNESS in es", async () => {
      const result = await sql`
        SELECT value, is_fallback FROM infi_translated
        WHERE entity_type = 'question'
          AND entity_id = 'q_CONSCIOUSNESS'
          AND locale = 'es'
          AND field = 'title'
  ...
  describe("max_quote_words trigger (§5.5)", () => {
    it("rejects quote exceeding source max_quote_words", async () => {
      ...
      await sql`DELETE FROM infi_citation WHERE id = 'cit_TEST_LONG'`;
  ...
  describe("locale system", () => {
    it("enforces single master locale", async () => {
      const masters = await sql`
        SELECT count(*)::int AS cnt FROM infi_locale WHERE is_master = true
      `;
      expect(masters[0]?.cnt).toBe(1);
    });

    it("has en as master and es as active", async () => {
      const en = await sql`SELECT is_master, is_active FROM infi_locale WHERE code = 'en'`;
      expect(en[0]?.is_master).toBe(true);
      expect(en[0]?.is_active).toBe(true);

      const es = await sql`SELECT is_master, is_active FROM infi_locale WHERE code = 'es'`;
